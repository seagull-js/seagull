...
class DocumentTest {
  ...
  async 'simple document without anything works'() {
    ...
    expect(content).to.be.equal('<html><head><title data-react-helmet="true"></title><style id="styles-target"></style></head><body><div id="root">here yo are</div><script src="/assets/bundle.js"></script></body></html>')
  }
  @test
  async 'simple document with ga and analytics  works'() {
    process.env.config_mock = JSON.stringify({analytics:{ga:'UA-23423',enabled:true}})
    const content = renderToStaticMarkup(<Document content={'here yo are'}/>)
