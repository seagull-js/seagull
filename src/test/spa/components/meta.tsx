...
class BodyTest {
  ...
  async 'Meta writes to head'() {
    const content = renderToStaticMarkup(<><Meta title={'My title'} /><Head /></>);
    expect(content.indexOf('<title data-react-helmet="true">My title</title>')).to.be.above(0)
  }
  @test
  async 'Meta children are working'() {
    const content = renderToStaticMarkup(<><Meta title={'My title'}><meta name='viewport' content='width=device-width, initial-scale=1' /> </Meta><Head/></>);
