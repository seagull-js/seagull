...
export class Test extends BasicTest {
  ...
  async 'WriteFile and ReadFile work'() {
    const content = '<html />'
    await new S3.WriteFile('dev', 'index.html', content).execute()
    const file = await new S3.ReadFile('dev', 'index.html').execute()
  ...
  async 'WriteFile can be reverted'() {
    const content = '<html />'
    const writer = new S3.WriteFile('dev', 'index.html', content)
    await writer.execute()
    await writer.revert()
    const reader = new S3.ReadFile('dev', 'index.html')
