...
const bundlerArgs: any = JSON.parse(process.env.BUNDLER_ARGS!)
const adapter: any = () => {
  const bus = new EventEmitter()
  process.on('message', m => {
    switch (m.type) {
      case 'BundlePageEvent':
        bus.emit(BundlePageEvent, ...m.payload)
    }
  })

  bus.on(BundledPageEvent, (...args: any) =>
    process.send!({
      payload: args,
      type: 'BundledPageEvent',
    })
  )
  bus.on(LogEvent, (...args: any) =>
    process.send!({
      payload: args,
      type: 'LogEvent',
    })
  )
  bus.on(BundlePageErrorEvent, (...args: any) =>
    process.send!({
      payload: args,
      type: 'BundlePageErrorEvent',
    })
  )
  return bus
}

const pbs = new PageBundleService(adapter(), bundlerArgs)
